...
bool iAHRSROS::restart()
{
  ...
  usleep(1000000);
  // std::string ra_command = "ra\n";
  // double ra_data[max_data];
  // if (iAHRSROS::_SendRecv(ra_command.c_str(), ra_data, max_data) < 0)
  // {
  //   return false;
  // }
  // usleep(500000);
...
bool iAHRSROS::readSyncData(geometry_msgs::Vector3& linear_acceleration, geometry_msgs::Vector3& angular_velocity, geometry_msgs::Vector3& magnetic_field, geometry_msgs::Quaternion& orientation)
{
  ...
  if (iAHRSROS::_Recv(data, max_data) == max_data)
  {
    ...
    if (_calib)
    {
      //std::cout << "activate gp calibration" << std::endl;
      data[11] += 90;
    ...
    {
      //std::cout << "deactivate gp calibration" << std::endl;
      data[11] += 90;
